...
class RAGService:
    ...
    def __init__(
        ...
        # Get Databricks client for data retrieval
...
Answer:"""
    ...
    def query(
        ...
        # Step 1: Retrieve facilities from database
        retrieval_start = time.time()
        
        query_sql = f"SELECT * FROM {settings.facilities_table}"
        where_clauses = []
        
        if filters:
            if filters.get("region"):
                where_clauses.append(f"address_stateOrRegion = '{filters['region']}'")
            if filters.get("facility_type"):
                where_clauses.append(f"facilityTypeId = '{filters['facility_type']}'")
        ...
        facilities = self.db_client.fetch_all(query_sql)
